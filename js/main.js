...
$(window).on("load", function() {
    ...
    // Fix form height in signup & signin pages
    var formHolder = $(".form-holder", "#form-section");
    var signForm = $(".signin-signup-form","#form-section");
    
    if ($(window).width() < 769) {
        formHolder.css("height",signForm.height()+160);
    }else{
        formHolder.removeAttr("style");
    }
    if(signForm.height()>formHolder.height()){
        formHolder.css("overflow","scroll");
    }else{
        formHolder.css("overflow","inherit");
    }
    ...
    // Fix form height in signup & signin pages
    var miniHeader = $("#mini-header");
    var formSection = $("#form-section");
    
    if ($(window).width() > 769) {
        formSection.css("height",$(window).height()-miniHeader.height());
    }else{
        formSection.removeAttr("style");
    }
    // Fix form height in signup & signin pages
    $(window).on("resize",function() {
        if ($(window).width() > 769) {
